 ...
 * Also by enabling the GlobalMethodLevelSecurity we emphasize the authentication and authorization for each method
 * of Rest Controller.
 */
@EnableWebSecurity
@Configuration
...
public class JPASecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests()
                .antMatchers("/h2-console/**", "/swagger-resources/**").permitAll()
                .anyRequest().authenticated()
                .and().csrf().ignoringAntMatchers("/h2-console/**", "/swagger-resources/**").disable().headers()
                .frameOptions().disable()
                .and().httpBasic().and().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS);
    }

    // if you don't provide a password encoder, spring security will throw a RuntimeException:
    ...
    /**
     * We are using JDBC(h2) based User Config
     *
